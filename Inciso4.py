...
# Convertir la solución exacta a una función de numpy
sol_exacta_np = sp.lambdify(x, sol_exacta.rhs, 'numpy')
print("Solucion Exacta EDO 1: ", sol_exacta)
...
plt.plot(x_heun_1, y_heun_1, 'g-.', label='Heun')
...
y_vals = np.linspace(-15, 15, 60)
X, Y = np.meshgrid(x_vals, y_vals)
U = np.ones_like(X)
V = f1(X, Y)

plt.figure(figsize=(10, 6))
plt.quiver(X, Y, U, V, angles='xy', scale_units='xy', scale=6, color='lightgray')
plt.plot(x_heun_1, y_heun_1, 'g-', label='Heun')
plt.xlabel('x')
plt.ylabel('y')
plt.title('Campo vectorial de la EDO y solución numérica (Heun)')
plt.legend()
plt.grid(True)
...
'''# Mostrar solución exacta en el intervalo [0, 5]
y_exact_values = sol_exacta_np(np.linspace(0, 5, 6))
for i, val in enumerate(y_exact_values):
    print(f"y({i}) = {val:.6f}")'''
...
plt.figure(figsize=(10, 6))
plt.quiver(X, Y, U, V, angles='xy', scale_units='xy', scale=10, color='gray')
plt.plot(x_heun_1, y_heun_1, 'g-', label='Heun')
plt.xlabel('x')
plt.ylabel('y')
plt.title('Campo vectorial de la EDO y solución numérica (Heun)')
